  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 pl-2.5 pr-3.5 py-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] hover:border-grey-400 transition-all duration-200"
      >
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-left min-w-0">
        <span className={`block font-medium text-sm ${isSelected ? "" : "text-[var(--color-text-primary)]"}`}>
          {account.name ?? "Account"}
        </span>
        <span className="block text-xs opacity-60 mt-0.5">
          {account.source} &middot; {truncateAddress(account.address)}
        </span>
      </span>
        ...
        <div className="p-5">
          {wallets.length === 0 ? (
            <div className="text-center py-8">
              <p className="font-medium text-[var(--color-text-primary)] mb-2">No wallets detected</p>
            ...
            <div className="space-y-2">
              {wallets.map((wallet) => (
                ...
                >
                  ...
                  <svg className="w-5 h-5 text-[var(--color-text-tertiary)] -rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// ── Inline SVG icons (avoid adding lucide-react dep) ─────────────────
